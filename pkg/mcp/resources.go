...
### community_list
List communities (label propagation) with anchors, top tags, and top authority notes. Response: ` + "`" + `{communities:[{id,size,fractionOfVault,anchor,topTags,topAuthority,density,bridges,bridgesDetailed}], stats, orphanCount, orphans?, components:[{id,size,fractionOfVault}]}` + "`" + `. Use the ` + "`" + `id` + "`" + ` for community_detail.
...
### community_detail
Show full detail for a community by ` + "`" + `id` + "`" + ` (from community_list/graph_stats) or by ` + "`" + `file` + "`" + ` (vault-relative/absolute path): anchor, density, bridges (with counts), fractionOfVault, top tags/authority, and members with hub/authority/in/out (optional tags/neighbors with linksIn/linksOut and bridge flags). Response: ` + "`" + `{id,anchor,size,fractionOfVault,density,bridges,bridgesDetailed,topTags,topAuthority,members[{path,title,inbound,outbound,hub,authority,community,scc,weakComponent,isBridge?,bridgeEdges?,tags?,neighbors?,linksOut?,linksIn?}],internalEdges}` + "`" + `.
...
### vault_context
Compact vault summary to orient an agent: graph stats, orphan counts, weak components, top communities (size/fraction/density/anchor/top tags & notes/bridges), key notes (anchors/bridges/top authority), optional MOC/key-note list from patterns (with pattern info), and optional embedded note_context payloads. Response: ` + "`" + `{stats, orphanCount, topOrphans?, components?[{id,size,fractionOfVault}], communities:[{id,size,fractionOfVault,anchor,density,topTags,topAuthority,bridgesDetailed}], keyNotes?, mocs?[{path,pattern?}], keyPatterns?, noteContexts?}` + "`" + `.
