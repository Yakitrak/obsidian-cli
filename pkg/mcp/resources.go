...
### community_list
List communities (label propagation) with anchors, top tags, and top pagerank notes. Response: ` + "`" + `{communities:[{id,size,fractionOfVault,anchor,topTags,topPagerank,density,bridges,bridgesDetailed}], stats, orphanCount, orphans?, components:[{id,size,fractionOfVault}]}` + "`" + `. Use the ` + "`" + `id` + "`" + ` for community_detail.
...
### community_detail
Show full detail for a community by ` + "`" + `id` + "`" + ` (from community_list/graph_stats) or by ` + "`" + `file` + "`" + ` (vault-relative/absolute path): anchor, density, bridges (with counts), fractionOfVault, top tags/pagerank, and members with pagerank/in/out (optional tags/neighbors with linksIn/linksOut and bridge flags). Response: ` + "`" + `{id,anchor,size,fractionOfVault,density,bridges,bridgesDetailed,topTags,topPagerank,members[{path,title,inbound,outbound,pagerank,community,scc,weakComponent,isBridge?,bridgeEdges?,tags?,neighbors?,linksOut?,linksIn?}],internalEdges}` + "`" + `.
...
### note_context
Return graph + community context for one or more notes (contexts follow input order). Response: ` + "`" + `{contexts:[...],count}` + "`" + `. Fields per note: ` + "`" + `{path,title?,error?,tags?,frontmatter?,graph:{inbound,outbound,pagerank,pagerankPercentile,isOrphan,weakComponent,strongComponent},community:{id,size,fractionOfVault,density,anchor,topTags,topPagerank,bridges,isBridge},neighbors:{linksOut:[{path,community}],linksIn:[...],sameCommunity,crossCommunity},neighborsTruncated?,neighborsLimit?,backlinks?,backlinksTruncated?,backlinksLimit?}` + "`" + `. If a file is not found in the graph, the response includes ` + "`" + `error` + "`" + ` instead of failing the batch. Neighbors and backlinks are sorted by PageRank (descending) before truncation, so the most important are kept.
...
### vault_context
Compact vault summary to orient an agent: graph stats, orphan counts, weak components, top communities (size/fraction/density/anchor/top tags & notes/bridges), key notes (anchors/bridges/top PageRank), optional MOC/key-note list from patterns (with pattern info), and optional embedded note_context payloads. Response: ` + "`" + `{stats, orphanCount, topOrphans?, components?[{id,size,fractionOfVault}], communities:[{id,size,fractionOfVault,anchor,density,topTags,topPagerank,bridgesDetailed}], keyNotes?, mocs?[{path,pattern?}], keyPatterns?, noteContexts?}` + "`" + `.
