...
**Options:** ` + "`" + `skipAnchors` + "`" + `, ` + "`" + `skipEmbeds` + "`" + `, ` + "`" + `includeTags` + "`" + `, ` + "`" + `recencyCascade` + "`" + ` (cascade inferred recency beyond 1 hop; default true), plus ` + "`" + `maxCommunities` + "`" + `, ` + "`" + `maxTopNotes` + "`" + `.
...
**Options:** ` + "`" + `files` + "`" + ` (required) plus ` + "`" + `skipAnchors` + "`" + `, ` + "`" + `skipEmbeds` + "`" + `, ` + "`" + `includeFrontmatter` + "`" + `, ` + "`" + `includeBacklinks` + "`" + ` (default true), ` + "`" + `includeNeighbors` + "`" + `, ` + "`" + `includeTags` + "`" + `, ` + "`" + `neighborLimit` + "`" + ` (default 50), ` + "`" + `backlinksLimit` + "`" + ` (default 50), ` + "`" + `recencyCascade` + "`" + ` (default true).

### vault_context
Compact vault summary to orient an agent: graph stats, orphan counts, weak components, top communities (size/fraction/density/anchor/top tags & notes/bridges with hub/authority scores + adaptive authorityBuckets with examples + authorityStats mean/p50/p75/p90/p95/p99/max + recency: latest note and recent count in last 30d), key notes (anchors/bridges/top authority), optional MOC/key-note list from patterns (with pattern info), optional embedded note_context payloads, and optional ` + "`" + `semanticMatches` + "`" + ` (chunk-level with headings/breadcrumbs) when ` + "`" + `semanticQuery` + "`" + ` is provided and embeddings are enabled. Response: ` + "`" + `{stats, orphanCount, topOrphans?, components?[{id,size,fractionOfVault}], communities:[{id,size,fractionOfVault,anchor,density,topTags,topAuthority,authorityBuckets,authorityStats,recency,bridgesDetailed}], keyNotes?, mocs?[{path,pattern?}], keyPatterns?, noteContexts?, semanticQuery?, semanticMatches?}` + "`" + `.

**Options:** ` + "`" + `skipAnchors` + "`" + `, ` + "`" + `skipEmbeds` + "`" + `, ` + "`" + `includeTags` + "`" + `, ` + "`" + `recencyCascade` + "`" + ` (default true), ` + "`" + `maxCommunities` + "`" + `, ` + "`" + `communityTopNotes` + "`" + `, ` + "`" + `communityTopTags` + "`" + `, ` + "`" + `bridgeLimit` + "`" + `, ` + "`" + `topOrphans` + "`" + `, ` + "`" + `topComponents` + "`" + `, ` + "`" + `topNotes` + "`" + `, ` + "`" + `keyPatterns` + "`" + ` (defaults to vault config keyNotePatterns), ` + "`" + `contextFiles` + "`" + `, contextIncludeBacklinks/frontmatter/neighbors/tags, ` + "`" + `contextNeighborLimit` + "`" + `, ` + "`" + `contextBacklinksLimit` + "`" + `, ` + "`" + `semanticQuery` + "`" + ` (text search via embeddings), ` + "`" + `semanticLimit` + "`" + ` (default 10).
